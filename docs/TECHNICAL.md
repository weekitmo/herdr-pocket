...
**两条列表都记着最后一份真读到的东西**，这是同一条规则的两次落地（都**按机器 id 键控** ——
绝不把上一台机器的树/看板当成答案）：

- `BoardNotifier._lastBoard`（`lib/data/providers/connection.dart`）：连接不在线时返回它 ——
  空白等于说"agent 都没了"，而真相是"连接没了"。⚠️ **凡是一次成功的读都要写它**：事件与
  30s 安全网都走 `refresh()`，只记 `build()` 那一次会让"先失败、后成功"的会话在重连时清空。
- `NavTreeCache`（`lib/data/providers/nav_tree.dart`）：工作区树是 **auto-dispose**（它挂在
  看板事件流上，永活会在每个事件突发多打 3 个 socket 请求），所以记忆**必须在 provider 之外**，
  否则页面一被 `RootShell` 换掉就失忆 —— 用户报的就是这个：
  「切换 nav tab 或者从另外的页面回来时，原本的列表就不见了」。`build()` 先把缓存写进
  `state`（第一帧就有内容、不闪空），再用真读到的结果替换；非 `Online` 时返回缓存；
  `refresh()` 成功也写缓存。
