...
**自己更新自己。** 设置 → *检查更新* 会去 GitHub 问有没有新版本，带进度条下载 APK，
进度条可以取消（取消会**保留已下载的部分**，下次接着下），下完对着 release 自带的
`checksums.txt` 校验一遍，再交给系统安装器。打开 *自动检查更新* 就是每次启动问一次，
有新版本时只说一句话。下载会跟随手机设置的 HTTP 代理 —— 没代理时慢的连接，配了就不慢。
...
- **只支持原版 herdr。** Gram 消息、他们的推送通知、联邦机器都在一个 fork 里，
  这里不支持。
- **应用内更新只在 Android 上能装。** 检查到处都能跑，但只有 Android 有办法把
  下好的 APK 交给系统安装器。macOS 上面板到发布页就停了 —— 何况 macOS 包本来
  就是未签名的，手动替换是唯一的路。
